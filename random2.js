...
// This script performs a sequence of 7 tasks, repeats 10 times with a random interval,
// and then waits ~24 hours to start again.
...
// ==========================================================
const RUNS_PER_SESSION = 12;

// --- NEW Random Delay Between Tasks ---
const MIN_TASK_DELAY_SEC = 5;  // Minimum 5 seconds
const MAX_TASK_DELAY_SEC = 30; // Maximum 30 seconds

// --- Random Interval Between Sequences ---
const BASE_INTERVAL_HOURS = 1;   // The 1-hour base wait
const MIN_RANDOM_MINUTES = 2;    // Minimum random minutes to add (e.g., 1h 2m)
const MAX_RANDOM_MINUTES = 10;   // Maximum random minutes to add (e.g., 1h 10m)

// --- Random Daily Jitter ---
const DAILY_JITTER_MINUTES = 5;  // Max random minutes to add to the 24h cycle (0-5)
// ==========================================================
...
// ... (code for task_deployContract - no changes)
...
// ... (code for task_createToken - no changes)
...
// ===================================================================================
// TASK 3: MINT NFT
// ... (code for task_mintNft - no changes)
...
// ... (code for task_claimAndBurn & sendSubtask - no changes)
...
// ===================================================================================
// TASK 5: SWAP TOKENS
// ... (code for task_swapTokens - no changes)
...
// ... (code for task_createMintpadNFT - no changes)
async function task_createMintpadNFT() {
    ...
    const TEMPLATE_DATA = '';
...
// ===================================================================================
// TASK 7: BRIDGE TOKENS
// ... (code for task_bridgeTokens - no changes)
async function task_bridgeTokens() {
    console.log("\n--- TASK 7: Bridging Tokens ---");
    
    // Using the raw data you provided for the bridge task
    await sendSubtask(
        '0x0000000000000000000000000000000000000900', 
        ''
    );
}

// ===================================================================================
// MASTER SCHEDULER (MODIFIED)
// ===================================================================================
async function scheduleDailyRuns() {
    ...
    console.log(`This bot will run a session of ${RUNS_PER_SESSION} task sequences.`);
    console.log(`Interval between sequences: ${BASE_INTERVAL_HOURS}h + ${MIN_RANDOM_MINUTES}-${MAX_RANDOM_MINUTES} random minutes.`);
    console.log(`Delay between tasks: ${MIN_TASK_DELAY_SEC}-${MAX_TASK_DELAY_SEC} random seconds.`);
    ...
    const tasks = [
        ...
        task_createToken,
        task_mintNft,
        task_claimAndBurn, // Renamed and updated to 4 steps
        task_swapTokens,
        task_createMintpadNFT, // New task
        task_bridgeTokens      // New task
    ];

    while (true) {
        // This is the line that captures the *exact* start time of the session.
        // For example: 11:45 PM, Oct 24
        const sessionStartTime = new Date();
        console.log(`\n\n>>> Starting new session at ${sessionStartTime.toLocaleString()} <<<`);

        for (let i = 1; i <= RUNS_PER_SESSION; i++) {
            console.log(`\n--- Running Sequence #${i} of ${RUNS_PER_SESSION} ---`);
            ...
            for (const task of tasks) {
                ...
                // ==========================================================
                // === MODIFIED TASK DELAY LOGIC ===
                // ==========================================================
                
                // 1. Generate random delay seconds (5-30)
                const randomDelaySec = getRandomInt(MIN_TASK_DELAY_SEC, MAX_TASK_DELAY_SEC);
                const randomDelayMS = randomDelaySec * 1000;
                
                // 2. Log and wait
                console.log(`  ... waiting ${randomDelaySec}s ...`);
                await new Promise(resolve => setTimeout(resolve, randomDelayMS));
            }
            
            const sequenceEndTime = Date.now();
            console.log(`--- Sequence #${i} finished in ${(sequenceEndTime - sequenceStartTime) / 1000} seconds. ---`);

            if (i < RUNS_PER_SESSION) {
                // ==========================================================
                // === RANDOM INTERVAL LOGIC (Unchanged) ===
                // ==========================================================
                
                // 1. Generate random minutes for this specific interval
                const randomMinutes = getRandomInt(MIN_RANDOM_MINUTES, MAX_RANDOM_MINUTES);
                
                // 2. Calculate total wait time in milliseconds
                const baseIntervalMS = BASE_INTERVAL_HOURS * 60 * 60 * 1000;
                const randomIntervalMS = randomMinutes * 60 * 1000;
                const totalWaitMS = baseIntervalMS + randomIntervalMS;

                // 3. Update the log message
                console.log(`\n...Waiting for the next sequence. See you in ${BASE_INTERVAL_HOURS}h ${randomMinutes}m...`);
        ...
        // ==========================================================
        // === DAILY SCHEDULING LOGIC (Unchanged) ===
        // ==========================================================
        
        // 1. Create a new Date object based on the *original* start time of this session.
        //    (e.g., If sessionStartTime was 11:45 PM, Oct 24)
        ...
        //    (e.g., nextSessionTime is now 11:45 PM, Oct 25)
        nextSessionTime.setDate(nextSessionTime.getDate() + 1);

        // 3. Add a small random "jitter" to the 24-hour cycle (e.g., 0-5 minutes)
        const dailyJitterMinutes = getRandomInt(0, DAILY_JITTER_MINUTES); 
        const dailyJitterMS = dailyJitterMinutes * 60 * 1000;
        nextSessionTime.setTime(nextSessionTime.getTime() + dailyJitterMS);

        // 4. Calculate the *exact* number of milliseconds from "now" until that future time.
        ...
        console.log(`Next session scheduled to start at: ${nextSessionTime.toLocaleString()} (includes ~${dailyJitterMinutes}m random delay)`);
        console.log(`Waiting for approximately ${waitHours} hours and ${waitMinutes} minutes to maintain the ~24-hour cycle.`);
        
        // 5. Wait for that exact duration.
